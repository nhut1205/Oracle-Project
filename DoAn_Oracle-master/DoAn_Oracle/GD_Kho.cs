...
namespace DoAn_Net
{
    public partial class GD_Kho : Form
    {
        ...
        {
            string connection1 = @"Data Source=localhost:1521/orcl;User ID=minhtri;Password=123;";
            ...
            if (!string.IsNullOrEmpty(txt_SoLuong.Text))
            {
                if (int.TryParse(txt_SoLuong.Text, out int parsedValue))
                {
                    ...
                    using (OracleConnection connection = new OracleConnection(connection1))
                    {
                        connection.Open();
                        OracleCommand command = new OracleCommand(quantityQuery, connection);
                        OracleDataReader reader = command.ExecuteReader();
                        if (reader.Read())
                        {
            ...
            if (dataGrid_BanHang.SelectedRows.Count > 0)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    using (OracleConnection connection = new OracleConnection(connectionString))
                    {
                        ...
                        {
                            command.Parameters.Add("@OrderDetailId", OracleDbType.Varchar2).Value = OrderetailID;
