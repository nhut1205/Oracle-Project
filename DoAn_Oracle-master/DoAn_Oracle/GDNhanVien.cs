...
namespace DoAn_Net
{
    public partial class GDNhanVien : Form
    {
                ...
                else
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        using (OracleConnection connection = new OracleConnection(connectionString))
                        {
                            ...
                            using (OracleCommand userDetailsCommand = new OracleCommand(deleteUserDetailsQuery, connection))
                            {
                                //userDetailsCommand.Parameters.AddWithValue("@TenDangNhap", tenDangNhap);
                                userDetailsCommand.Parameters.Add(new OracleParameter("TenDangNhap", tenDangNhap));
                                //userDetailsCommand.Parameters.Add("@TenDangNhap", tenDangNhap);
                                ...
                                if (userDetailsRowsAffected > 0 && quyen != "1")
                                {
                                    ...
                                    using (OracleCommand accountCommand = new OracleCommand(deleteAccountQuery, connection))
                                    {
                                        //accountCommand.Parameters.AddWithValue("@TenDangNhap", tenDangNhap);
                                        accountCommand.Parameters.Add(new OracleParameter("TenDangNhap", tenDangNhap));
                                        //accountCommand.Parameters.Add("@TenDangNhap", tenDangNhap);
        ...
        //                    using (OracleCommand checkOrdersCommand = new OracleCommand(checkOrdersQuery, connection))
        //                    {
                ...
                try
                {
                    using (OracleConnection connection = new OracleConnection(connectionString))
                    {
                        ...
                        else
                        {
                            ...
                            using (OracleCommand checkOrdersCommand = new OracleCommand(checkOrdersQuery, connection))
                            {
                                checkOrdersCommand.Parameters.Add(new OracleParameter("TenDangNhap", OracleDbType.Varchar2));
                                checkOrdersCommand.Parameters["TenDangNhap"].Value = tenDangNhap;
                                ...
                                else
                                {
                                    ...
                                    using (OracleCommand userDetailsCommand = new OracleCommand(deleteUserDetailsQuery, connection))
                                    {
                                        userDetailsCommand.Parameters.Add(new OracleParameter("TenDangNhap", OracleDbType.Varchar2));
                                        userDetailsCommand.Parameters["TenDangNhap"].Value = tenDangNhap;
                                        ...
                                        if (userDetailsRowsAffected > 0)
                                        {
                                            ...
                                            using (OracleCommand accountCommand = new OracleCommand(deleteAccountQuery, connection))
                                            {
                                                accountCommand.Parameters.Add(new OracleParameter("TenDangNhap", OracleDbType.Varchar2));
                                                accountCommand.Parameters["TenDangNhap"].Value = tenDangNhap;
